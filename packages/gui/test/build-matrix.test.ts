 ...
 * check-no-auto-publish.mjs` (`npm run verify:no-publish`, in the CI `verify` job).
 *
 * The INVARIANT itself — every distributable-producing invocation disables publishing — is
 * asserted by `core/test/gold/no-auto-publish.test.ts` through that real scanner, which parses
 * invocations instead of regexing the script string. This block owns only the other half: the
 * scanner stays WIRED into the PR path (process.html R05 "protect the guard").
 */

 ...
 *
 * Scope: the TRAY only. The popover's own surface IS drivable headlessly — the judge loads
 * popover.html in Chromium — so per engineering.html §05 the scene is the guard there:
 * JUDGE `TRAY_POPOVER_SURFACE` (Stop/Start + Open Stint present, no `#switch`, both states)
 * and `TRAY_COUNTUP` (the running clock), never a regex over the popover source.
 */

...
export function createIpcHandlers(deps: IpcHandlerDeps): IpcHandlers {
  ...
  const handlers: IpcHandlers = {
    ...
    exportEntries: (payload) => {
      ...
      // report shows (byte-identical to `tt report run <name> --csv|--json`), 'all' writes the WHOLE
      // RECORD — every raw entry ever, no range (byte-identical to `tt export` with no flags).
      // resolveExportDefinition owns the split; either way the bytes come from core's
      // toCsv/toJsonEntries and write through the OS save dialog. No network.
      const now = new Date();
      const { fileDayUtc, entries } = resolveExportDefinition(payload, store, now);
      const bytes = exportPayload(entries, payload.format, now);
      const target = deps.showSaveDialog(payload.format, exportFileName(fileDayUtc, payload.format));

export const meta = {
  ...
  whenToUse:
    ...
    'Pass args to scope a run to a subset of requirements (e.g. args: ["§09 search", "§12 R8"]) ' +
    'for a cheap calibration pass or a staged sequence; omit args for the full sweep.',
  phases: [
...
// scopeTo: narrow this run to a subset of requirements. `args` may be a string,
// an array of strings, or { scopeTo: [...] }. Each token is matched (case-insensitive,
// substring) against "reqId section title", so "§12 R8", "search", or "settings" all work.
// Omit args for the full sweep. Lets you stage coverage as a sequence of cheap, reviewable
// runs (no mid-run input) instead of one large autonomous job.
...
let work = inv.items;
if (scope) {
  work = work.filter((w) => scope.some((s) => `${w.reqId} ${w.section} ${w.title}`.toLowerCase().includes(s)));
...
log(`Inventory: ${work.length} open requirements (${work.filter(w => w.statusNow === 'todo').length} todo, ${work.filter(w => w.statusNow === 'partial').length} partial); ${(inv.evidenceGaps || []).length} evidence gaps${scope ? ' (scoped)' : ''}.`);

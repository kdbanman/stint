...
`npm run judge`, `npm run evidence`, `npm run verify:no-network`, `npm run icons`.
...
| `context/design.html` | **The look, made law.** Normative visual spec: the two-layer token system, colour roles, type ramp, spacing grid, component grammar (accent discipline, selection, fields, messages, icons), the WCAG 2.2 AA floors, the app mark (§09), and the split verification stance (computed checks + rendered comparison). Values live in `design.tokens.json`; the mockups illustrate it. |
...
| `context/mark/` | **The face.** The three hand-authored SVG sources behind every icon the OS draws — `mark.svg` (the app icon), `glyph-idle.svg` / `glyph-running.svg` (the tray pair). They carry classes, not fills: `npm run icons` (`scripts/gen-icons.mjs`) injects the palette from `design.tokens.json` and rasterizes every size, and the PNGs are committed because rasterizing needs Chromium. Rules in `context/design.html` §09. |
| `context/mockups/` | **The spec, illustrated.** Standalone, dependency-free HTML mockups of the GUI views (`timer`, `main`, `reports`, `settings`, `edit-entry`, `clients`, `tray-popover`, `software-update`, `merge-conflict`, `merge-gap`, `storage-change`) — illustrations of `context/prd.html` §12 (behavior/layout) and `context/design.html` (style). They show only specified reality; their token blocks are generated from `design.tokens.json`. Keep in sync when GUI requirements change (PRD §18). |
...
| `acceptance/` | The acceptance-criteria apparatus, split in two: `criteria/` (coverage matrix, UI-state inventory, JSON schemas, JUDGE rubric, MANUAL runbook, parity matrix — **what must hold**) and `evidence/` (generated **proof those criteria hold** — the tracked text evidence: CLI transcript, judge report, recordings index). |

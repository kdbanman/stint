...
Entries-toolbar control is touched (the range presets retired with the week-only
view, so the prev-week stepper latches the renderer's entries query exactly as
the Today preset used to), routing to the Timer view and clicking Start flips the card to

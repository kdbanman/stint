...
| CHECK CONFIRM DESTRUCTIVE (GUI) (§12 R13) | JUDGE `CONFIRM_DELETE`, `CONFIRM_ARCHIVE`, `CONFIRM_DESTRUCTIVE`, `RESTORE_ARCHIVED` — the shipped `confirmInline` gate driven through the real renderer, asserting the first click arms without calling `remove` and only the explicit confirm fires it once (issue #167 deleted the uncalled model this used to cite); BDD `features/overlap_and_editing.feature` "Deleting an entry without confirmation is refused". |
...
| CHECK DESTRUCTIVE CONFIRM + LIVE FILTER (§17 R11) | JUDGE `CONFIRM_DESTRUCTIVE`, `LIVE_FILTER`; BDD `features/overlap_and_editing.feature` "Deleting an entry without confirmation is refused" + `features/entry_list.feature` "Per-day and range billable totals over the week — including an empty day". |
...
| CHECK VERSION-SKEW REFUSAL (§20 R09) | GOLD `packages/core/test/gold/migration.test.ts` ("refuses to open a DB from a NEWER schema…" and "Store.open refuses the same future-stamped DB… and writes no launch backup") + `packages/cli/test/gold/cli.test.ts` ("tt status on a future-stamped DB exits 1 with both versions and the remedy on stderr, leaving the directory unchanged") + GOLD `packages/gui/test/schemaskew.test.ts` (the **GUI surface**: a real future-stamped DB's refusal becomes an error box naming both versions, the remedy and the refused file; no other open failure is claimed; main performs the box then exits non-zero). |

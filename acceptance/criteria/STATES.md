...
| **Error** — broken config / dead configured DB parent at launch (§20 R10/R11) | NATIVE dialog naming the file + error / the configured path + config file, offering Reset to default / Quit; no fallback opened | GOLD `gui/test/storage.test.ts` (the decision logic); MANUAL runbook `CHECK STORAGE CHANGE` step 3 | Native chrome — no headless host; deliberately no mockup (the R05 dialog convention) |

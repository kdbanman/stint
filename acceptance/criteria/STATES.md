...
| **Ideal** — favorites rail populated | One row per favorite, Resume + kebab, monochrome chrome; the cards share one width in an equal-width wrap grid, the partial last row keeping it (D07, issue #354) | JUDGE `FAVORITES_RAIL` — `timer-favorites.png` | |
| **Edge** — pin-as-favorite form | The rail header's Pin — the view's ONE pin affordance (issue #353) — swaps into the inline name field; submit fires `pinFavorite` from both states: the open row's template while running, the Start form's fields while idle | JUDGE `FAVORITES_RAIL` (the inline pin driven to completion in both states; exactly one Pin-labeled button on the view) — `timer-favorites.png`, `timer-favorites-empty.png` | |
| **Error** — refused pin (duplicate name) | The pin form's announced warning region names the refusal — the reason alone (issue 138); the form stays open, the rail unchanged (§12 R21) | JUDGE `FAVORITES_RAIL` (a re-pin of a seeded name driven to the refusal) — `timer-favorites-pin-refused.png` | Swallowed (`catch {}`) until issue #353 — every refused pin read as a dead button |

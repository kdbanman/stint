...
| **Edge** — pending-changes gate (§12 R24) | A dirty subject swap (another event, an empty-spot create, an external refresh) blocks on the keep-editing / discard dialog; Keep editing preserves all seven pending fields, each kind arming it alone; only Discard swaps; a clean form swaps in place | JUDGE `PENDING_CHANGES_GATE` — `main-pending-gate.png`; `UNIFIED_FORM` (`cleanSwap`) | DOM-only gate: no BDD leg can see it (COVERAGE.md's DOM-gate rule) — this row + the JUDGE row are its proof |

...
| **Edge** — fine-snap toggle | Ephemeral switch in the + button's spot; on → drags land on the fine grid; never persisted, coarse on every open | JUDGE `UNIFIED_FORM_ADD` (`fineSnapOk`) | Deliberately not a setting (§14) |
...
| **Edge** — pending-changes gate (§12 R24) | A dirty subject swap (another event, an empty-spot create, an external refresh) blocks on the keep-editing / discard dialog; Keep editing preserves every pending field, only the explicit Discard swaps; a clean form swaps in place with no prompt | JUDGE `PENDING_CHANGES_GATE` — `main-pending-gate.png`; `UNIFIED_FORM` (`cleanSwap`) | DOM-only gate: no BDD leg can see it (COVERAGE.md's DOM-gate rule) — this row + the JUDGE row are its proof |

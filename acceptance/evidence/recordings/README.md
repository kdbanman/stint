...
| §12 R16 | `12-r16.gif` | Entries calendar: a week of equal day columns at their comfortable floor width (spare window width is shared — §12 R22) with per-day header totals + range chip, horizontal scroll, vertical scroll to off-hours (scroll, never clip), and hover ops on an event; empty days as empty columns. |
| §12 R09 / §17 R11 | `12-r09.gif` | Entries toolbar over a multi-week fixture (issue #55): each control — search (range + search compose), every range preset, the billable toggle, client + project, tag — visibly narrows the calendar AND moves the `#week-total` chip (week-bounded 5.00h idle, never the all-time sum); ends with the on-camera wire verdict that every `listEntries` query carried `by:'day'` with zero throws. Mirrors the hardened `ENTRIES_CALENDAR` / `LIVE_FILTER` judge scenes. |

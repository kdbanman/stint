...
| §12 R17 | `12-r17.gif` | Exact / overnight entry through the form's ALWAYS-PRESENT raw Start/Stop fields (the collapsed expander is retired): open the form from the keyboard path, type an overnight span (22:00 → 02:00 next day) and watch the grid repaint it as one segment per day — Wednesday's running to the column foot, Thursday's from the head down to 02:00 — then Save: the overnight entry persists, counted under its start day. |
...
| §12 R15 | `12-r15.gif` | The start-only interval picker — the requirement's whole scope now that manual add lives on the grid: the Timer view's Start field discloses it IN FLOW (no modal, no Apply) on the exact stored start, with one start grip, no end control anywhere, the block dissolving into the future and the ephemeral fine-snap toggle beside the track; dragging the grip writes the Start field live. Ends by asserting the other half of the requirement — the Entries view mounts NO picker: a closed entry's span drags on the grid's own edge grips or is typed in the raw fields. |
| §12 R16 | `12-r16.gif` | The Entries week grid: equal day columns FILLING the view with no horizontal scroll (§12 R22 — the floor width and its scrollbar retired with the week-only view), per-day billable totals in the headers and no range chip anywhere, vertical scroll to the off-hours entries (scroll, never clip), and hover ops on an event; empty days as empty columns. |
...
| §09 R01 | `09-r01.gif` | Reports custom range as two plain date fields driving the run output — the one range surface left in the GUI; then Entries, where the recipe asserts on camera that no preset segment and no date field exists at all (week-only, §12 R09) and steps the week to show the only range concept the view has. |
